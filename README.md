...
Hệ thống theo dõi kèo realtime từ `8xbet/default` và `jun88/cmd`, xác nhận lại cơ hội trực tiếp tại collector rồi phát qua dashboard và Chrome Notification.

## Thành phần

- `collector/`: hai Playwright worker, network-first cho 8xbet và DOM stream cho Jun88 CMD.
- `backend/`: Go API, collector WebSocket ingest, Redis current-state và detector.
