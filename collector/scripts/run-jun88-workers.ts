...
const enabledLobbies = envString("JUN88_WORKER_LOBBIES", "bti,ibc,cmd,m8")
  .split(",")
  .map((value) => value.trim())
  .filter(Boolean) as LobbyCode[];

const lobbyCollectorIDs: Record<LobbyCode, string> = {
  default: "jun88-default",
  bti: "jun88-bti",
  ibc: "jun88-ibc",
  cmd: "jun88-cmd",
  m8: "jun88-m8"
...
async function main() {
  ...
  await Promise.all(enabledLobbies.map((lobbyId) => runWorkerSafely(lobbyId, sink)));
}
...
async function runWorker(lobbyId: LobbyCode, sink: BackendCollectorSink) {
  const collectorId = lobbyCollectorIDs[lobbyId];
  const collector = createJun88LobbyCollector(collectorId, lobbyId as "bti" | "ibc" | "cmd" | "m8");

  if ("stream" in collector && typeof collector.stream === "function" && lobbyId === "bti") {
...
async function runWorkerSafely(lobbyId: LobbyCode, sink: BackendCollectorSink) {
  const collectorId = lobbyCollectorIDs[lobbyId];
...
main().catch((error) => {
  console.error("[jun88-workers] fatal:", error);
  process.exit(1);
});
